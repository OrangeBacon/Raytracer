...
impl<T: Number> AnimatedTransform<T> {
    ...
    fn decompose(mat: &Matrix4x4) -> Option<(Vector3<T>, Quaternion<T>, Matrix4x4)> {
        ...
        for _ in 0..=100 {
            ...
            for i in 0..4 {
                for j in 0..4 {
                    r_next[i][j] = 0.5 * (rotate[i][j] + r_inv[i][j])
                }
            }
    ...
    pub fn motion_bounds(&self, bound: Bounds3f) -> Bounds3f {
        if !self.is_animated {
...
impl DerivativeTerm {
    fn eval(&self, point: Point3f) -> Float {
        self.0 + point.to_vec().dot(Vector3f::new(self.1, self.2, self.3))
    }
}
...
fn calculate_derivative_terms<T: Number>(
    ...
    let mut c2: [DerivativeTerm; 3] = Default::default();
    let mut c3: [DerivativeTerm; 3] = Default::default();
    let mut c4: [DerivativeTerm; 3] = Default::default();
    ...
    c2[1] = DerivativeTerm(
        ...
        qperpx * qperpy * s000 + qperpw * qperpz * s000 + q0z * q0z * s010
            ...
            + q0x
                * (q0w * s020 + q0y * (-s000 + s100) - q0w * s120 + 2.0 * qperpy * s000 * theta
        ...
        qperpx * qperpy * s001 + qperpw * qperpz * s001 + q0z * q0z * s011
            ...
            + q0x
                * (q0w * s021 + q0y * (-s001 + s101) - q0w * s121 + 2.0 * qperpy * s001 * theta
        ...
        qperpx * qperpy * s002 + qperpw * qperpz * s002 + q0z * q0z * s012
            ...
            + q0x
                * (q0w * s022 + q0y * (-s002 + s102) - q0w * s122 + 2.0 * qperpy * s002 * theta
